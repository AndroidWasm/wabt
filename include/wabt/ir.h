...
struct Module {
  ...
  // instructions) to their correpsonding function indices.
  std::unordered_map<Offset, Index>
      function_index_by_function_pointer_load_offset_;

  // Mappings (ordered) from offsets of 32- and 64-bit function pointers within
  // global data initializers to their corresponding function indices.
  std::map<Offset, Index> function_index_by_fptr32_init_offset_;
  std::map<Offset, Index> function_index_by_fptr64_init_offset_;

  // Mapping from offsets of memory pointer loads (operands of ixx.const
  // instructions) to their corresponding data segment indices.
  std::unordered_map<Offset, Index>
      data_segment_index_by_memory_pointer_load_offset_;

  // Mapping from (ordered) offsets of 32- and 64-bit memory ppointers within
  // global data initializers to their corresponding data segment indices.
  std::map<Offset, Index> data_segment_index_by_mptr32_init_offset_;
  std::map<Offset, Index> data_segment_index_by_mptr64_init_offset_;
};
